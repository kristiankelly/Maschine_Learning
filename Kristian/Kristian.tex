...
\subsection{Musikgenerierung mit Recurrent Neural Networks}
...
\end{figure}
Wie bereits im ersten Abschnitt erwähnt, wollen wir die Notenwerte in einem Vektor darstellen, was wir in unserem Preprocessing dann auch tun. Somit wird jedes Stück nun als eine 
...
Gegeben eine Anfangssequenz von Noten $[n_1,\ldots,n_L]$, können wir das Modell verwenden, um die nächsten $N$ Noten vorherzusagen. Dabei erweist es sich als nützlich,
...
wir strukturell für den Rest des Projekts verwenden werden, siehe Abbildung \ref{fig:Einfaches_RNN_N=4,L=32,O=1}.
\begin{figure}[h]
  \centering
...
\begin{itemize}
    \item Wir verwenden eine Embedding-Schicht, um die Notenwerte in einen dichteren Vektorraum (in diesem Fall $\mathbb{R}^{64}$) abzubilden, was dem Netzwerk helfen soll, siehe z.B. \cite{chollet2021learning_ferdi} 
    Kapitel 14. Es ist nämlich bei Noten wie bei Sprachen so, dass Noten, deren Abstände sehr kurz sind (z.B. C und C\#) nicht unbedingt eine ähnliche Bedeutung/Wirkung haben.
          Somit muss das Netzwerk diese Beziehungen selbst lernen.
    \item Wir verwenden zwei LSTM (Long Short-Term Memory) Zellen, welche sehr vereinfacht gesagt in der Lage sind, Langzeitabhängigkeiten in Sequenzen zu lernen, siehe z.B. \cite{chollet2021learning_ferdi} Kapitel 13 für eine ausführliche Beschreibung.
    ...
    Da die höchsten und tiefsten Notenwerte im Dataset nicht vorkommen, können wir auf 88 reduzieren, also die Tasten eines Klaviers.
\end{itemize}

Wir trainieren für 20 Epochen mit Early Stopping und erhalten eine Val-Accuracy von ca. $0.82$ und eine Test-Accuracy von ca. $0.80$. NICHT SCHLECHT! Ein generiertes Musikstück sieht man 
...
\end{figure}
Uns fallen zwei Dinge auf. Zum einen werden die Akkorde oft wiederholt, was jedoch zu erwarten war, da wir die rythmische Komponente komplett ignorieren, weswegen eine ganze Note
...
wie etwa die authentische Kadenz in den Takten 6-7. Siehe \cite{amon2005lexikon}, Kapitel über Harmonie und die Funktionalität in Dur und Moll.
...
\subsubsection{Zweiter Versuch mit $N=1$}
...
hat sich an strikte harmonische Regeln gehalten, wie etwa den Kontrapunkt, siehe \cite{tittel1959neue}. Als nächsten Schritt wollen wir versuchen, diese Regeln irgendwie in das 
Modell zu integrieren, um konsistenter harmonische Stücke zu generieren.
\subsubsection{Eine neue Loss Funktion}
...
Wir bemerken, dass der zweite Term genau dann 0 ist, wenn die Note korrekt vorhergesagt wurde, und umso größer wird, je 'dissonanter' die Vorhersage ist, also genau das, was wir wollten.
...
Wir haben uns also dahingehend nicht wirklich verbessert. Aber wie schauen die generierten Musikstücke aus? Nun ja, wir haben unser Ziel erfüllt: Die generierten Stücke sind harmonisch perfekt. 
Leider bedeutet 'perfekt' in diesem Fall, dass die ganze Zeit nur der erste Akkord (die Tonika) gespielt wird. Das gibt uns zwar das objektiv beste harmonische Stück,
